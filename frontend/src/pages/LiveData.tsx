...
function LiveData() {
  ...
  // Safe faults rendering - prevents crash if faults is string, null, etc.
  const faultsDisplay = Array.isArray(liveData.faults)
    ? liveData.faults.join(" • ")
    : typeof liveData.faults === 'string'
      ? liveData.faults.replace(/[\[\]"]/g, '').trim() || 'None reported'
      : 'None reported';
  ...
  return (
    <div style={{
      ...
      <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px", maxWidth: "800px", margin: "0 auto", color: faultsDisplay === 'None reported' ? "#aaa" : "#fecaca" }}>
        <strong>⚠️ FAULTS:</strong> {faultsDisplay}
      </div>
    </div>
  );
}
