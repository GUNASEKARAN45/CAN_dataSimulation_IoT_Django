...
export default function Signup() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.post("/api/auth/signup/", { username, email, password, confirm_password: confirmPassword, role });
  ...
  const labelStyle: React.CSSProperties = {
    ...
    fontWeight: "600", color: "#475569", letterSpacing: "0.06em", textTransform: "uppercase",
  };
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif", padding: "20px" }}>
        <div style={{ background: "#ffffff", borderRadius: "16px", padding: "40px 44px", boxShadow: "0 1px 4px rgba(0,0,0,0.06), 0 8px 32px rgba(0,0,0,0.08)", border: "1px solid #eef2f7", width: "100%", maxWidth: "440px", animation: "fadeUp 0.35s ease both" }}>
          ...
          <h2 style={{ margin: "0 0 6px", fontSize: "1.4rem", fontWeight: "700", color: "#0f172a", textAlign: "center" }}>Create account</h2>
          <p style={{ margin: "0 0 24px", color: "#64748b", fontSize: "0.875rem", textAlign: "center" }}>Fill in the details below to get started</p>

          {error   && <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "10px 14px", marginBottom: "16px", color: "#dc2626", fontSize: "0.83rem", fontWeight: "500" }}>⚠ {error}</div>}
          {success && <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "8px", padding: "10px 14px", marginBottom: "16px", color: "#16a34a", fontSize: "0.83rem", fontWeight: "500" }}>✓ {success}</div>}

          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <div><label style={labelStyle}>Username</label>         <input className="auth-input" type="text"     value={username}        onChange={e => setUsername(e.target.value)}        style={inputStyle} /></div>
            <div><label style={labelStyle}>Email</label>            <input className="auth-input" type="email"    value={email}           onChange={e => setEmail(e.target.value)}           style={inputStyle} /></div>
            <div><label style={labelStyle}>Password</label>         <input className="auth-input" type="password" value={password}        onChange={e => setPassword(e.target.value)}        style={inputStyle} /></div>
            <div><label style={labelStyle}>Confirm Password</label> <input className="auth-input" type="password" value={confirmPassword}  onChange={e => setConfirmPassword(e.target.value)} style={inputStyle} /></div>

            <div>
              <label style={labelStyle}>Role</label>
              <select className="auth-input" value={role} onChange={e => setRole(e.target.value)} style={{ ...inputStyle, cursor: "pointer", appearance: "auto" }}>
          ...
          <p style={{ marginTop: "24px", textAlign: "center", fontSize: "0.875rem", color: "#64748b" }}>
            ...
            {/* React Router Link — changes URL to /login */}
            <Link to="/login" style={{ color: "#505bfe", fontWeight: "600", textDecoration: "none", fontFamily: "'DM Sans', sans-serif" }}>
              Sign in
            </Link>
          </p>
        </div>
