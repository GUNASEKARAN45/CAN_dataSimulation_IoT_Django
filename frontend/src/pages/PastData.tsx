    ...
    <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
      {[
        ...
        <div key={label} style={{
          ...
          <span style={{ fontSize: "0.65rem", color: "#94a3b8", fontWeight: "500", letterSpacing: "0.06em", textTransform: "uppercase" }}>{label}</span>
          <span style={{
            fontSize: "0.95rem",
...
function PastData() {
  ...
  const fetchData = async (date?: string) => {
    ...
    try {
      ...
      if (Array.isArray(json) && json.length === 0) {
        setNoDataMessage(date ? `No data available for ${new Date(date).toLocaleDateString()}` : "No data available in the last 5 days");
        setData([]);
      } else {
        setData(json);
        setNoDataMessage("");
  ...
  const avgBattGain = data.length ? data.reduce((s, d) => s + (d.battery_gain ?? 0), 0) / data.length : 0;
  ...
  return (
    <>
      ...
      <div style={{ padding: "32px 40px", maxWidth: "1200px", margin: "0 auto", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        {/* Summary cards */}
        {data.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "16px", marginBottom: "28px", animation: "fadeUp 0.4s ease both 0.05s" }}>
            {[
              ...
              <div key={label} style={{ background: "#ffffff", borderRadius: "12px", padding: "18px 20px", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                <p style={{ margin: "0 0 6px", fontSize: "0.7rem", color: "#94a3b8", fontWeight: "600", letterSpacing: "0.1em", textTransform: "uppercase" }}>{label}</p>
                <div style={{ display: "flex", alignItems: "baseline", gap: "4px" }}>
                  <span style={{ fontSize: "2rem", fontWeight: "700", color: "#0f172a", fontFamily: "'Roboto Mono', monospace", lineHeight: 1 }}>{value}</span>
        ...
        {/* Filter controls */}
        <div style={{ background: "#ffffff", borderRadius: "14px", padding: "20px 24px", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", marginBottom: "24px", display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap", animation: "fadeUp 0.4s ease both 0.1s" }}>
          <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "#475569", letterSpacing: "0.02em" }}>Filter by Date</label>
          <input
          ...
          {selectedDate && (
            <button className="action-btn" onClick={() => { setSelectedDate(""); fetchData(); }} style={{ padding: "9px 16px", background: "transparent", color: "#64748b", border: "1px solid #e2e8f0", borderRadius: "8px", cursor: "pointer", fontSize: "0.875rem", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s ease" }}>
              ✕ Clear
            </button>
          )}
        </div>
        ...
        {/* Loading */}
        {loading && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "12px", padding: "60px 0", color: "#64748b" }}>
            <div style={{ width: "20px", height: "20px", borderRadius: "50%", border: "2px solid #e2e8f0", borderTopColor: "#3b82f6", animation: "spin 0.7s linear infinite" }}/>
        ...
        {noDataMessage && !loading && (
          <div style={{ background: "#ffffff", borderRadius: "14px", padding: "60px 40px", textAlign: "center", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.06)", animation: "fadeUp 0.3s ease both" }}>
        ...
        {!loading && !noDataMessage && data.length > 0 && (
          <div style={{ background: "#ffffff", borderRadius: "16px", border: "1px solid #f1f5f9", boxShadow: "0 1px 3px rgba(0,0,0,0.07), 0 8px 24px rgba(0,0,0,0.04)", overflow: "hidden", animation: "fadeUp 0.4s ease both 0.15s" }}>
            {/* Header */}
            <div style={{ display: "grid", gridTemplateColumns: "140px 160px 160px 1fr 1fr", background: "#f8fafc", borderBottom: "2px solid #f1f5f9" }}>
              {["Date", "Distance", "Battery Gain", "Motor Temp (°C)", "Battery Temp (°C)"].map((h, i) => (
                <div key={i} style={{ padding: "14px 16px", fontSize: "0.7rem", fontWeight: "700", color: "#64748b", letterSpacing: "0.1em", textTransform: "uppercase", borderRight: i < 4 ? "1px solid #f1f5f9" : "none", textAlign: i >= 3 ? "center" : "left" }}>
                  {h}
                  {i >= 3 && (
                    <div style={{ display: "flex", justifyContent: "space-around", marginTop: "4px" }}>
                      {["Min", "Avg", "Max"].map(sub => (
                        <span key={sub} style={{ fontSize: "0.6rem", color: "#94a3b8", fontWeight: "500" }}>{sub}</span>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div key={index} className="data-row" style={{ display: "grid", gridTemplateColumns: "140px 160px 160px 1fr 1fr", borderBottom: index < data.length - 1 ? "1px solid #f8fafc" : "none", background: "#ffffff", animation: `rowIn 0.3s ease both ${index * 40}ms`, transition: "background 0.15s ease" }}>
                ...
                <div style={{ padding: "0 16px", display: "flex", alignItems: "center", borderRight: "1px solid #f1f5f9" }}>
                  <div>
                    ...
                    <div style={{ fontSize: "0.72rem", color: "#94a3b8" }}>{new Date(day.date).getFullYear()}</div>
                  </div>
                </div>
